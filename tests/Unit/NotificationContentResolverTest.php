...
it('allowedChannelsFor intersects override with type ceiling in database mode', function (): void {
    config(['notifications-max.content_source' => 'database']);
...
it('allowedChannelsFor drops override entries the type no longer supports', function (): void {
    config(['notifications-max.content_source' => 'database']);

    NotificationTypeOverride::query()->create([
        'tenant_id' => null,
        'type_key' => 'invoice.paid',
        'allowed_channels' => ['push', 'sms'], // sms is stale — type doesn't allow it
    ]);

    expect($this->resolver->allowedChannelsFor('invoice.paid', tenantId: null))
        ->toBe(['push']); // sms filtered out by intersection
});
