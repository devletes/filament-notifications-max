...
class GenericNotification extends Notification
{
    ...
    public function buildFilamentPayload(): array
    {
        ...
        // Read by the redirect controller at click time.
    ...
    /**
     * Back-compat URL builder. Resolution: context.action_url → redirect
     * route (when an address exists) → direct ActionUrlBuilder call.
     */
    ...
    /**
     * Resource address for the redirect controller. Sourced from
     * `context.action` (polymorphic types) or synthesized from the
     * registry (`actionResource` + `actionRecordKey`).
     */
    public function buildActionAddress(NotificationType $type): ?NotificationActionAddress
    {
        ...
        // Mirror the pre-multi-panel single-panel default when only
        // target_panel is declared.
        $panels = $type->panels ?? ($type->targetPanel !== '' ? [$type->targetPanel] : []);
