...
return [
    ...
    */
    ...
    'type_defaults' => [
        ...
        // Preferred panel for the action URL — tie-breaker when the user
        // has access to multiple `panels`, and the sole panel when
        // `panels` is omitted.
        'target_panel' => 'admin',
        // Panels that can render this record. Null = no registry-level
        // constraint (polymorphic types supply the address per dispatch).
        // Example: ['admin', 'employee'] for cross-panel resources.
        'panels' => null,
        'category' => 'general',
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Click-time Redirect Route
    |--------------------------------------------------------------------------
    |
    | Action clicks go through `GET /{prefix}/go/{notification}?from={panel}`
    | which resolves the destination panel at click time. The bell and
